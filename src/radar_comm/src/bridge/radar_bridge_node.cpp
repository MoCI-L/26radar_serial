...
RadarBridgeNode::RadarBridgeNode()
    ...
    map_robot_data_pub_ = this->create_publisher<radar_interfaces::msg::MapRobotData>(
        "/radar/rx/map_robot_data", data_qos);
    decision_cmd_pub_ = this->create_publisher<radar_interfaces::msg::RadarDecisionCommand>(
    ...
    watchdog_timer_ = this->create_wall_timer(500ms, [this]() { watchdog_check(); });
...
void RadarBridgeNode::reload_runtime_parameters() {
    ...
    publish_queue_warn_threshold_ = this->get_parameter("publish_queue_warn_threshold").as_int();
    ...
    reconnect_cpu_affinity_ = this->get_parameter("reconnect_cpu_affinity").as_int();
...
rcl_interfaces::msg::SetParametersResult RadarBridgeNode::on_parameter_change(
    ...
    if (need_reconnect) {
        RCLCPP_INFO(this->get_logger(), "Runtime serial parameters changed, reconnecting");
        disconnect_serial();
    }
    return result;
}
...
void RadarBridgeNode::connect_serial() {
    ...
    if (!serial->open(port, baud)) {
        RCLCPP_WARN(this->get_logger(), "Failed to open serial port %s", port.c_str());
        return;
    }
    ...
    RCLCPP_INFO(this->get_logger(), "Connected serial %s @ %d", port.c_str(), baud);
}
...
void RadarBridgeNode::maybe_record_raw(const uint8_t *data, std::size_t size) {
    ...
    if (raw_record_stream_.is_open()) {
        raw_record_stream_.write(reinterpret_cast<const char *>(data), static_cast<std::streamsize>(size));
...
void RadarBridgeNode::maybe_log_hex(const uint8_t *data, std::size_t size) {
    ...
    RCLCPP_DEBUG(this->get_logger(), "RX HEX [%zu]: %s", size, bytes_to_hex(data, size).c_str());
}

void RadarBridgeNode::handle_serial_chunk(const uint8_t *data, std::size_t size) {
    ...
    try {
        ...
        parser_.process(data, size, [this, stamp](radar_comm::ProtocolParser::ParsedFrame &&frame) {
            ...
            if (!publish_queue_.push(std::move(envelope))) {
                runtime_stats_.queue_overflows.fetch_add(1, std::memory_order_relaxed);
                return;
            }
            publish_cv_.notify_one();
        });

        if (static_cast<int>(publish_queue_.size()) > publish_queue_warn_threshold_) {
            RCLCPP_WARN_THROTTLE(
                this->get_logger(), *this->get_clock(), 2000,
                "Publish queue depth high: %zu", publish_queue_.size());
        }
...
void RadarBridgeNode::watchdog_check() {
    ...
    const auto age = std::chrono::duration_cast<std::chrono::milliseconds>(
        now.time_since_epoch() - std::chrono::nanoseconds(last_rx_ns)).count();
...
void RadarBridgeNode::publish_status() {
    ...
    msg.watchdog_triggered = runtime_stats_.watchdog_triggered.load(std::memory_order_relaxed);
    msg.port = this->get_parameter("port").as_string();
    msg.baud_rate = static_cast<uint32_t>(this->get_parameter("baud_rate").as_int());
    ...
    msg.queue_overflows = runtime_stats_.queue_overflows.load(std::memory_order_relaxed);
    msg.reconnect_attempts = runtime_stats_.reconnect_attempts.load(std::memory_order_relaxed);
    msg.serial_disconnects = runtime_stats_.serial_disconnects.load(std::memory_order_relaxed);
    msg.parse_exceptions = runtime_stats_.parse_exceptions.load(std::memory_order_relaxed);
    msg.publish_exceptions = runtime_stats_.publish_exceptions.load(std::memory_order_relaxed);
    msg.last_rx_age_ms = static_cast<uint64_t>(
        std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::steady_clock::now().time_since_epoch() -
            std::chrono::nanoseconds(last_rx_steady_ns_.load(std::memory_order_relaxed))).count());
...
void RadarBridgeNode::on_protocol_data(const ProtocolEnvelope &envelope) {
    std::visit([this, &envelope](auto &&arg) {
...
void RadarBridgeNode::publish_position(const radar_comm::EnemyRobotPosition &pos, const builtin_interfaces::msg::Time &stamp) {
...
void RadarBridgeNode::publish_hp(const radar_comm::EnemyRobotHP &hp, const builtin_interfaces::msg::Time &stamp) {
...
void RadarBridgeNode::publish_ammo(const radar_comm::EnemyRemainingAmmo &ammo, const builtin_interfaces::msg::Time &stamp) {
...
void RadarBridgeNode::publish_macro_status(const radar_comm::EnemyTeamMacroStatus &status, const builtin_interfaces::msg::Time &stamp) {
...
void RadarBridgeNode::publish_buffs(const radar_comm::EnemyRobotBuffs &buffs, const builtin_interfaces::msg::Time &stamp) {
...
void RadarBridgeNode::publish_key(const radar_comm::EnemyInterferenceKey &key, const builtin_interfaces::msg::Time &stamp) {
...
void RadarBridgeNode::publish_mark_progress(const radar_comm::RadarMarkProgress &progress, const builtin_interfaces::msg::Time &stamp) {
...
void RadarBridgeNode::publish_radar_info(const radar_comm::RadarInfo &info, const builtin_interfaces::msg::Time &stamp) {
...
void RadarBridgeNode::publish_map_robot_data(const radar_comm::MapRobotData &data, const builtin_interfaces::msg::Time &stamp) {
...
void RadarBridgeNode::publish_decision_command(const radar_comm::RadarDecisionCommand &cmd, const builtin_interfaces::msg::Time &stamp) {
